...
void BreakoutScene::Init()
{
	...
	{
		if (m_GameState == BreakoutGameState::InServe)
		{
	...
	{
		if (m_GameState == BreakoutGameState::Play || m_GameState == BreakoutGameState::InServe)
		{
			if (Core::GameController::IsPressed(state))
...
{
	m_GameState = BreakoutGameState::InServe;
	m_Ball.Stop();
